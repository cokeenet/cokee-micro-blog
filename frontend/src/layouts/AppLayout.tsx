    ...
    return (
        <div className="bg-background text-on-background min-h-screen font-inter">
            <div className="max-w-[1280px] mx-auto flex min-h-screen relative">
                {/* SideNavBar */}
                <aside className="hidden lg:flex w-64 h-screen sticky top-0 border-r border-slate-200 dark:border-sky-300/10 bg-white/80 dark:bg-slate-950/60 backdrop-blur-xl flex-col py-4 px-6 gap-2 shadow-[0_0_30px_rgba(125,211,252,0.05)]">
                    <div className="mb-8 px-2 flex items-center gap-2">
                        <span className="material-symbols-outlined text-3xl text-sky-500 dark:text-sky-300">cookie</span>
                        <span className="text-2xl font-black text-sky-500 dark:text-sky-300 tracking-tight">咕咕饼干</span>
                    </div>
                        ...
                        {navigation.map((item) => {
                            ...
                            return (
                                <Link
                                    key={item.name}
                                    to={item.path}
                                    className={`flex items-center gap-4 px-4 py-3 font-inter text-base rounded-full transition-all duration-200 active:scale-95 ${isActive
                                        ? 'text-sky-500 dark:text-sky-300 font-bold bg-sky-500/10 dark:bg-sky-300/10'
                                        : 'text-slate-600 dark:text-slate-400 font-regular hover:bg-slate-100 dark:hover:bg-slate-800/50'
                                        }`}
                                >
                                    <span
                ...
                {/* Main Content */}
                <main className="flex-1 min-w-[600px] border-r border-slate-200 dark:border-sky-300/10">
                    {children}
                </main>
